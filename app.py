...
.step-card::before {
    ...
    width: 3px; border-radius: 14px 0 0 14px; background: rgba(255,255,255,0.05);
}
.step-card.active::before { background: #ff8c32; }
.step-card.done::before   { background: #50c878; }
...
<div class="hero">
    <div class="hero-eyebrow">Multi-Agent AI System</div>
    <h1>Research<span>Mind</span></h1>
    <p class="hero-sub">Four specialized AI agents collaborate — searching, scraping, writing, and critiquing — to deliver a polished research report on any topic.</p>
</div>
<div class="divider"></div>
""", unsafe_allow_html=True)
...
# ── Run pipeline — imports happen HERE, not at top ───────────────────────────
if run_btn:
    if not topic.strip():
        st.warning("Please enter a research topic first.")
    else:
...
if st.session_state.running and not st.session_state.done:
    try:
        # ✅ Import only when actually needed — UI already rendered above
        from agents import build_reader_agent, build_search_agent, writer_chain, critic_chain
